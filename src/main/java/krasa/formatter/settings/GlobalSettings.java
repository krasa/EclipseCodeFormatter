...
public class GlobalSettings
    ...
    public Settings newSettings() {
        ...
        aNew.setBackupToProjectConfigFile(false);
    ...
    @NotNull
    public Settings findGlobalProfile(@NotNull Settings state, @NotNull Project project) throws DeletedProfileException {
        if (state.isNotSaved()) {
    ...
    public Settings findGlobalProfile(GlobalProfileReference globalProfileReference, Project project) throws DeletedProfileException {
        for (Settings settings : settingsList) {
            if (settings.getId().equals(globalProfileReference.getId())) {
                return settings;
            }
        }
        for (Settings settings : settingsList) {
            if (settings.getName().equals(globalProfileReference.getName())) {
                return settings;
            }
        }

        if (deletedSettingsId.contains(globalProfileReference.getId())) {
            throw new DeletedProfileException();
        }
